...
import { API_URL, STATE_MOD, TPL_URL } from '../utils/constants';
import type { TModalOptions, TTemplateData } from '../utils/types';
  ...
  async fetchItems(id: string, modalOverlay: HTMLElement | null = null) {
    ...
    try {
      ...
      if(success && isSucceed) {
        const item = this.parseData(resData as Record<string, string>, tpl as Template);
